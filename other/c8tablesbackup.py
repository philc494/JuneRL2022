...
def qtables_8(train_pattern, test_pattern, train_iterations, test_iterations):
    ...
    for a in A_int_offlimits:
        for i in range(-1, 2):
            for j in range(-1, 2):
                rewards_A_int[a][(i, j)] = 'off'
    for a in B_int_offlimits:
        for i in range(-1, 2):
            for j in range(-1, 2):
                rewards_B_int[a][(i, j)] = 'off'
    for a in C_int_offlimits:
        for i in range(-1, 2):
            for j in range(-1, 2):
                rewards_C_int[a][(i, j)] = 'off'
    for a in D_int_offlimits:
        for i in range(-1, 2):
            for j in range(-1, 2):
                rewards_D_int[a][(i, j)] = 'off'
            ...
            if np.random.uniform(0, 1) <= exp_rate:
                ...
                for a in int_actions:
                    if prev_target == "A":
                        ...
                        print(poss_reward)
                        print("this is Target A, game {}".format(game))
                        print("current position is: {}".format(current_pos))
                        print("I am trying to take action {}".format(a))
                        print("this is int move {}".format(int_move_counter))
                        if poss_reward > best_reward:
                            next_int_action = a
                            best_reward = poss_reward
                    elif prev_target == "B":
                        poss_reward = rewards_B_int[take_next_move(
                            a)][int_action_trans[a]]
                        print(poss_reward)
                        print("this is Target B, game {}".format(game))
                        print("current position is: {}".format(current_pos))
                        print("I am trying to take action {}".format(a))
                        print("this is int move {}".format(int_move_counter))
                        if poss_reward > best_reward:

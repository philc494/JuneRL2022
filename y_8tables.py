...
deterministic = True  # todo: incorporate
z_config.scenario_num = '1'
        ...
        if current_pos == win_pos:
            ...
            if (game+1) % (games/10) == 0:
                print("Game {} of {} completed:  Act moves in last game: {}".format(
                    ...
                    games,
                    act_move_counter))
            z_config.moves_per_train.append(act_move_counter)
            z_config.game_num_train.append(game + 1)
            z_config.scenario_per_train.append(scenario)
            game_reset()
...
Model evaluation
"""""""""""""""""""""""""""""""""""""""""""""""""""
if input('Test trained model? (yes/no) ') == 'yes':
    test_flag = True
    if input('Same scenario pattern as training? (yes/no) ') == 'yes':
        test_iterations = int(input('How many iterations? '))
    else:
        win_pattern = input("Please input desired scenario pattern: ")
        test_iterations = int(input('How many iterations? '))

    win_seq = win_pattern * test_iterations
    games = len(win_seq)
    game = 0
    current_pos = (2, 2)
    into_int_state = False
            ...
            if current_pos == win_pos:
                if (game + 1) % (games / 10) == 0:
                    print("Game {} of {} completed:  Act moves in last game: {}".format(
                        ...
                        games,
                        act_move_counter))
                z_config.moves_per_test.append(act_move_counter)
                z_config.game_num_test.append(game + 1)
                z_config.scenario_per_test.append(scenario)
                game_reset()
                prev_scenario = scenario
            else:  # take another move in action state
                act_action = pick_act_move(scenario)
...
"""""""""""""""""""""""""""""""""""""""""""""""""""
visualize_tables(rewards_A, rewards_B, rewards_C, rewards_D,
rewards_A_int, rewards_B_int, rewards_C_int, rewards_D_int)

dftrain = z_statistics.train_stats()
if test_flag:
    dftest = z_statistics.test_stats()
    z_statistics.plot_statistics_both(dftrain, dftest)
else:
    z_statistics.plot_statistics_train(dftrain)
    print("Done")
    exit()
print("Done")

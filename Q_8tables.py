...
act_action_trans = {"up": (-1, 0), "down": (1, 0), "left": (0, -1), "right": (0,1 ), "uleft": (-1, -1), "uright": (-1, 1), "dleft": (1, -1), "dright": (1, 1)}
int_action_trans = {"up": (-1, 0), "down": (1, 0), "left": (0, -1), "right": (0,1 ), "uleft": (-1, -1), "uright": (-1, 1), "dleft": (1, -1), "dright": (1, 1), "stay": (0, 0)}
    ...
    while True:
        ...
        if np.random.uniform(0, 1) <= exp_rate:
            ...
            for a in act_actions:
                if win_scenario == "A":
                    poss_reward = rewards_A[take_next_move(a)][act_action_trans[a]]
...
while game < games:
    ...
    if current_pos == win_pos:
        ...
        print("Game: {}  Target: {}  New pos: {}  Action: {}  Move #: {}".format(game + 1, win_pos, current_pos, act_action, act_move_counter + 1))
        current_pos = take_next_move(act_action)
        act_action_coord = act_action_trans[act_action]
        reward_positions[current_pos] = act_action_coord
        act_move_counter += 1

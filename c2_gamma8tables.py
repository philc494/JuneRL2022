...
def qtables_8gamma(train_pattern, test_pattern, train_iterations, test_iterations):
    ...
    def update_act_rewards(reward_apply):
        ...
        list_length = len(reversed_act_list)
        for x in range(0, list_length - 1):
        # for idx, x in enumerate(reversed_act_list):
            if scenario == "A":
                rewards_A[reversed_act_list[x][1]] = round(
                    rewards_A[reversed_act_list[x][1]] + alpha * (reward_apply - rewards_A[a][b]), 2)
            elif scenario == "B":
                rewards_B[a][b] = round(
